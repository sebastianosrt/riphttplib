...
async fn connect_socks5_proxy(
    ...
    stream.read_exact(&mut response).await.map_err(|e| {
        ProtocolError::ConnectionFailed(format!("SOCKS5 greeting response failed: {}", e))
    })?;
...
async fn socks5_authenticate(
    ...
    stream.write_all(&auth_request).await.map_err(|e| {
        ProtocolError::ConnectionFailed(format!("SOCKS5 auth request failed: {}", e))
    })?;

    let mut response = vec![0; 2];
    stream.read_exact(&mut response).await.map_err(|e| {
        ProtocolError::ConnectionFailed(format!("SOCKS5 auth response failed: {}", e))
    })?;
...
async fn socks5_connect(
    ...
    stream.write_all(&connect_request).await.map_err(|e| {
        ProtocolError::ConnectionFailed(format!("SOCKS5 connect request failed: {}", e))
    })?;
    ...
    match response[3] {
        0x01 => {
            ...
            stream.read_exact(&mut addr_port).await.map_err(|e| {
                ProtocolError::ConnectionFailed(format!("SOCKS5 IPv4 response read failed: {}", e))
            })?;
        }
        0x03 => {
            // Domain name
            ...
            stream.read_exact(&mut len_buf).await.map_err(|e| {
                ProtocolError::ConnectionFailed(format!("SOCKS5 domain length read failed: {}", e))
            })?;
            let domain_len = len_buf[0] as usize;
            let mut domain_port = vec![0; domain_len + 2]; // domain + 2 bytes port
            stream.read_exact(&mut domain_port).await.map_err(|e| {
                ProtocolError::ConnectionFailed(format!(
                    "SOCKS5 domain response read failed: {}",
                    e
                ))
            })?;
        }
        0x04 => {
            ...
            stream.read_exact(&mut addr_port).await.map_err(|e| {
                ProtocolError::ConnectionFailed(format!("SOCKS5 IPv6 response read failed: {}", e))
            })?;
        }
        _ => {
            return Err(ProtocolError::ConnectionFailed(
                "SOCKS5 unsupported address type in response".to_string(),
            ));
        }
    }
...
async fn connect_socks4_proxy(
                ...
                Ok(mut addrs) => {
                    if let Some(addr) = addrs.next() {
                        if let std::net::IpAddr::V4(ipv4) = addr.ip() {
                            ...
                            return Err(ProtocolError::ConnectionFailed(
                                "SOCKS4 only supports IPv4".to_string(),
                            ));
                        }
                    } else {
                        return Err(ProtocolError::ConnectionFailed(
                            "Failed to resolve hostname for SOCKS4".to_string(),
                        ));
                    }
                }
                Err(e) => {
    ...
    stream.write_all(&connect_request).await.map_err(|e| {
        ProtocolError::ConnectionFailed(format!("SOCKS4 connect request failed: {}", e))
    })?;
...
async fn connect_to_proxy_tcp(
    ...
    let stream = if let Some(timeout_duration) = connect_timeout {
        timeout(timeout_duration, connect_future)
            ...
            .map_err(|e| {
                ProtocolError::ConnectionFailed(format!("Failed to connect to proxy: {}", e))
            })?
    } else {
